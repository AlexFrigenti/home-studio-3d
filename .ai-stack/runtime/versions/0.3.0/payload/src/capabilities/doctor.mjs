...
function selectedRequirements(requirements, options) {
  const taskRequirements = options.task
    ? selectRequirements(requirements, [options.task]).requirements
    : requirements;
  if (!options.capability) return taskRequirements;
  return taskRequirements.filter(({ id }) => id === options.capability);
}

export function resolveDoctorScope({ requirements, catalog, options = {} }) {
  ...
  const selected = selectedRequirements(requirements, options);
...
export async function runDoctor({ projectRoot, options = {} } = {}) {
  ...
  const scope = resolveDoctorScope({ requirements: declarations.requirements, catalog, options });
  const requirements = selectedRequirements(declarations.requirements, options);
