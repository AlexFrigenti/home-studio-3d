...
function canonicalTaskIds(tasks) {
  return unique(tasks ?? []).sort();
}

export function selectRequirements(requirements, tasks, declaredTasks) {
  const requestedTasks = canonicalTaskIds(tasks);
  const knownTasks = new Set(declaredTasks ?? []);
  for (const taskId of requestedTasks) {
    if (!knownTasks.has(taskId)) throw new CapabilityError("TASK_UNKNOWN");
  }
...
export async function runPreflight({ projectRoot, options = {} } = {}) {
  ...
  const selection = selectRequirements(declarations.requirements, options.tasks ?? [], declarations.declaredTasks);
