...
import { inspectCanonicalProjectSkill } from "./project-skill.mjs";
...
export async function inspectProject(projectRoot) {
  ...
  const validateManifest = manifest.schemaVersion === 1
    ? validators.validateProjectManifestV1
    : validators.validateProjectManifest;
  if (!validateManifest(manifest) || !validateProjectManifestTaskReferences(manifest)) {
  ...
  const projectSkillPath = await assertRegularPath(projectRoot, manifest.projectSkill, "INVALID_PROJECT_SKILL");
  let projectSkillMetadata;
  try {
    projectSkillMetadata = inspectCanonicalProjectSkill(await readFile(projectSkillPath));
  } catch (error) {
    if (error?.code === "INVALID_PROJECT_SKILL_FORMAT") throw error;
    throw bootstrapError("INVALID_PROJECT_SKILL");
  }
  ...
  Object.defineProperty(facts, "projectSkillMetadata", {
    value: projectSkillMetadata,
    enumerable: false,
    writable: false,
    configurable: false
  });
