...
For a release candidate, the exact committed Git tree is the first-party byte authority. Release tooling reads the allowlisted blobs from that commit and then applies the existing byte-preserving snapshot builder. A working-tree representation, Git index, local path, or `.gitattributes` checkout transformation is not a release identity.
