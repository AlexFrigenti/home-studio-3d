...
function collectDeclaredTasks(manifest) {
  if (manifest.schemaVersion !== 2) return undefined;
  return [...manifest.tasks].sort();
}
...
export async function loadProjectCapabilityInputs(projectRoot, { catalog, resolveRequirements = true } = {}) {
  try {
    ...
    const validateManifest = manifest.schemaVersion === 1
      ? bootstrapValidators.validateProjectManifestV1
      : bootstrapValidators.validateProjectManifest;
    ...
    return {
      ...
      declaredTasks: collectDeclaredTasks(manifest),
