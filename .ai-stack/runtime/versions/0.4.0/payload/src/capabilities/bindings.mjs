...
import { resolvePrimitive, validatePrimitiveConfig } from "./primitives.mjs";
import { loadCapabilityValidators } from "./schema.mjs";
import { assertSafeTarget, DEFAULT_FILE_SYSTEM } from "./safe-paths.mjs";
import { parseExecutableVersion } from "./version-parser.mjs";
...
async function executableBinding(record, fileSystem, versionParsing) {
  ...
  return {
    status: "resolved",
    bindingHandle: createExecutableVersionHandle({ executable, versionParsing }),
    ...identity
  };
}
...
export function createProductiveBindingProvider({ catalog, storePath, fileSystem = DEFAULT_FILE_SYSTEM } = {}) {
  ...
  return Object.freeze({
    async resolveBinding({ symbolicTarget, targetClass, primitiveId, config, signal } = {}) {
      try {
        ...
        if (targetClass === "executable" && record.resource.kind === "executable-path") {
          return executableBinding(record, safeFileSystem, config?.["version-parsing"]);
        }
...
export function createExecutableVersionHandle({ executable, versionParsing = "exact-semver" } = {}) {
  ...
  return createOpaqueHandle({
    async readVersion({ timeoutMs, signal } = {}) {
      ...
      const version = parseExecutableVersion(result.stdout, versionParsing);
      return { version };
    }
  });
}
...
export async function resolveBinding({
  ...
  let result;
  try {
    result = await provider.resolveBinding({ symbolicTarget, targetClass, primitiveId, config, signal });
  } catch {
    throw new CapabilityError("CAPABILITY_BINDING_UNKNOWN");
  }
